...
def visualize_mean_face(x_mean, size, new_dims):
    ...
    return cv2.resize(x_mean.reshape(size), new_dims, interpolation = cv2.INTER_CUBIC)
...
def part_1c(p=0.5, k=5, random = False):
    ...
    if random:
    # random
        rand = np.random.randint(low=1, high=16, size=len(ytest))
        good = 0
        bad = 0
        ...
        print('random: Good predictions = ', good, 'Bad predictions = ', bad)
        print('{0:.2f}% accuracy'.format(100 * float(good) / (good + bad)))
...
def part_2a():
    ...
    count = 0
    for i in range(len(rand_y)):
        if rand_y[i] == ytrain[i]:
            count+=1
    rand_accuracy = 100 * float(count)/len(rand_y)
    ...
    for i in range(len(rand_y)):
        if rand_y[i] == ytrain[i]:
            count+=1
    wk_accuracy = 100 * float(count)/float(len(rand_y))
    ...
    #print(len(rand_y))
    #print(count)
    rand_accuracy = 100 * float(count)/float(len(rand_y))
    ...
    for i in range(len(Xtest)):
        if wk_results[i] == ytest[i]:
            count+=1
    wk_accuracy = 100 * float(count)/float(len(ytest))
    ...
    for iteration in range(1,9):
        ...
        for i in range(len(y_pred)):
            if ytest[i] == y_pred[i]:
                count+=1
        boost_accuracy = 100 * float(count)/float(len(y_pred))
...
def part_4_a_b():
    ...
    for i in range(len(predictions)):
        if predictions[i] == labels[i]:
            count += 1
    vj_accuracy = 100 * float(count)/float(len(predictions))
